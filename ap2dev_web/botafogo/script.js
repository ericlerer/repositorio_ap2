...
// Função para exibir as informações do jogador selecionado
function exibirInformacoes(id) {
  const jogador = jogadores[id];

  document.getElementById("posicao-jogador").textContent = jogador.posicao;
  document.getElementById("descricao-jogador").textContent = jogador.descricao;
  document.getElementById("nome-completo-jogador").textContent = jogador.nome_completo;
  document.getElementById("nascimento-jogador").textContent = jogador.nascimento;
  document.getElementById("altura-jogador").textContent = jogador.altura;
  document.getElementById("imagem-jogador").src = jogador.imagem;
}

// Função para obter o parâmetro da URL
function obterParametroURL() {
  const urlParams = new URLSearchParams(window.location.search);
  return urlParams.get("jogador");
}

// Função para encontrar o jogador correspondente
function encontrarJogador() {
  const jogadorNome = obterParametroURL();
  for (let i = 0; i < jogadores.length; i++) {
    if (jogadores[i].nome.toUpperCase() === jogadorNome.toUpperCase()) {
      return jogadores[i];
    }
  }
  return null;
...
// Função para preencher as informações do jogador na página
function preencherInformacoesJogador(jogador) {
  if (jogador) {
    const imagemElement = document.getElementById("imagem-jogador");
    const nomeElement = document.getElementById("nome-jogador");
    const posicaoElement = document.getElementById("posicao-jogador");
    const descricaoElement = document.getElementById("descricao-jogador");
    const nomeCompletoElement = document.getElementById("nome-completo-jogador");
    const nascimentoElement = document.getElementById("nascimento-jogador");
    const alturaElement = document.getElementById("altura-jogador");

    imagemElement.src = jogador.imagem;
    nomeElement.textContent = jogador.nome;
    posicaoElement.textContent = jogador.posicao;
    descricaoElement.textContent = jogador.descricao;
    nomeCompletoElement.textContent = jogador.nome_completo;
    nascimentoElement.textContent = jogador.nascimento;
    alturaElement.textContent = jogador.altura;
  }
}

// Função principal para carregar as informações do jogador
function carregarInformacoesJogador() {
  const jogador = encontrarJogador();
  preencherInformacoesJogador(jogador);
}

// Carregar as informações do jogador ao carregar a página
